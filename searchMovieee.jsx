...
const MovieSearch = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    await searchMovies();
    // Scroll to the movie grid after form submission
    document
      .getElementById("movie-grid")
      .scrollIntoView({ behavior: "smooth" });

    // Pass the search query as state
    navigate("/movie", { state: { query: query } });
  };
  ...
  return (
    <>
      ...
      <div className="items-center justify-center h-screen bg-landing ">
        ...
        <div
          id="movie-grid"
          className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 bg-landing"
        >
          {movies.map((movie) => (
            <div
              key={movie.id}
            ...
            >
              <div>
                ...
                <p className="text-gray-700">{limitWord(movie.overview, 20)}</p>
              </div>
